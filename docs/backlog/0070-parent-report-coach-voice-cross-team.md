...
- 2026-06-06 [implementation-dev] Shipped via PR #382 with all three gating checks green (`lint`, `unit-tests`, `e2e-tests`). Auto-merge armed after CI green per LESSONS#0075; shipped-flip filed as separate chore branch off freshly-pulled main per the same lesson. Local-gate vitest had ONE unrelated environmental TZ failure in `player-of-match-utils.test.ts` (Apr 27 vs Apr 28) — reproduces on bare `main` (LESSONS#0036), CI's UTC Node 20 runs it green. The route used the LESSONS#0103 optional-widening pattern: `CoachingSignature.voice_anchors` is OPTIONAL, so every existing 0037 / 0039 caller (practicePlan / practiceArc / pregame / newsletter / pulse / game-decompression) stayed byte-identical without a sweep. Three sibling test files (parent-report-continuity, parent-report-cross-season, parent-report-route-thin-week) had their `mockReturnValueOnce` queues extended in the same PR per LESSONS#0049 / #0092 / #0100 / #0110.
