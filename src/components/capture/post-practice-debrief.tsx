...
import { Check, Star, AlertTriangle, X, Loader2, ChevronRight, ChevronLeft } from 'lucide-react';
...
const positiveTemplates = ['Great energy', 'Strong passing', 'Good defense', 'Excellent hustle', 'Smart plays', 'Team leadership'];
const workTemplates = ['Ball handling', 'Spacing', 'Transitions', 'Communication', 'Shot selection', 'Defensive positioning'];

type Step = 'standouts' | 'positives' | 'work' | 'notes';
...
export function PostPracticeDebrief({ sessionId, onClose }: Props) {
  ...
  function togglePositive(t: string) {
    setPositives((prev) =>
      prev.includes(t) ? prev.filter((p) => p !== t) : [...prev, t]
    );
  }

  function toggleWork(t: string) {
    setNeedsWork((prev) =>
      prev.includes(t) ? prev.filter((p) => p !== t) : [...prev, t]
    );
  }
