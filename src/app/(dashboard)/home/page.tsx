...
interface QuickObserveModalProps {
...
function QuickObserveModal({
  ...
  return (
    ...
    >
      <div className="w-full rounded-t-2xl border-t border-zinc-800 bg-zinc-950 pb-safe max-h-[85vh] overflow-y-auto">
        {/* Drag handle */}
...
function LiveCoverageGrid({
  ...
  observedIds: string[];
  sessionId: string;
  onPlayerTap: (player: { id: string; name: string }) => void;
}) {
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-3 space-y-2.5">
      ...
      <div className="flex flex-wrap gap-1.5">
        {players.map((p) => {
          ...
          return observed ? (
            ...
            <button
              key={p.id}
              onClick={() => onPlayerTap({ id: p.id, name: p.name })}
      ...
      {!allCovered && (
        <p className="text-[10px] text-zinc-600">Tap a chip to log an observation instantly</p>
      )}
    </div>
  );
}
...
export default function HomePage() {
  ...
  const [quickObservePlayer, setQuickObservePlayer] = useState<{ id: string; name: string } | null>(null);
  ...
  return (
        ...
        <div className="space-y-3">
          ...
          {rosterPlayers.length > 0 && (
            <LiveCoverageGrid
              ...
              onPlayerTap={setQuickObservePlayer}
            />
          )}
        </div>
    ...
    {/* Quick-observe modal — opens when a player chip is tapped in the coverage grid */}
    {quickObservePlayer && practiceSessionId && activeTeam && coach && (
      <QuickObserveModal
        player={quickObservePlayer}
        teamId={activeTeam.id}
        coachId={coach.id}
        sessionId={practiceSessionId}
        sportId={(activeTeam as any).sport_slug ?? null}
        onClose={() => setQuickObservePlayer(null)}
        onSaved={() => setQuickObservePlayer(null)}
      />
    )}
    </>
  );
