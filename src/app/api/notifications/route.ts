...
export type NotificationType =
  ...
  | 'parent_viewed_report'; // parent opened player report card in last 24 hours
...
export async function GET(request: Request) {
  ...
  const [playersRes, obsRes, goalsRes, sessionsRes, achievementsRes, viewedSharesRes] = await Promise.all([
    admin
      .from('players')
  ...
  const viewedShares = viewedSharesRes.data ?? [];
