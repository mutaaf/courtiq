 ...
 * signature to thread into the prompt as a soft preference. Ticket 0039 added
 * the second argument: the coach's `coach_drill_signals` rows (their thumbs-up
 * / thumbs-down on individual drills) re-rank the signature's recurring drills
 * so up-rated drills float ahead of frequency-only matches and down-rated
 * drills are suppressed. Best-effort: any read failure resolves to null so
 * generation is never blocked, and a cold-start coach (too few plans) returns
 * null → the prompt is byte-identical to today. Reads only plan-derived fields
 * and the coach-private drill signals — no `players`/observation data (COPPA).
 */
async function fetchCoachingSignature(
  ...
  try {
    // Fetch plans, the coach's drill signals, and a drill_id→name lookup in
    // parallel; the signals fetch is the 0039 layer and is independent of the
    // plan history. A failure on either resolves to an empty array so the
    // signature still builds (best-effort).
    ...
    // Build a name lookup for the signals' drill_ids so the signature's
    // name-keyed recurring_drills re-ranks correctly. Only the rated drills
    // are looked up (the cardinality is small — the coach's lifetime ratings).
