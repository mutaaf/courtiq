...
export function buildMatchSessionLabel(
  ...
  // Parse YYYY-MM-DD as local time to avoid UTC-to-local date shift
  const dateStr = date ? new Date(date + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '';
  if (opponent) {
