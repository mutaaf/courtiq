/**
 * Ticket 0048 — `plans_type_check` constraint extension migration.
 *
 * The new migration widens the `plans.type` CHECK constraint allow-list to
 * include 'postgame_parent_texts'. It must:
 *   - reference `plans_type_check` and add EXACTLY the
 *     'postgame_parent_texts' value
 *   - touch no other table, column, or constraint
 *   - introduce NO new minor-data column (the texts are an existing-plan-shape
...
function findMigration(): { file: string; sql: string } {
  ...
  const match = files.find((f) => /plans.?type.?postgame.?parent.?texts/i.test(f));
  if (!match) throw new Error('No plans_type_postgame_parent_texts migration found in supabase/migrations');
...
describe('plans_type_postgame_parent_texts migration (ticket 0048)', () => {
  it("extends plans_type_check to include 'postgame_parent_texts'", () => {
    const { sql } = findMigration();
    const ddl = ddlOnly(sql).toLowerCase();
    expect(ddl).toContain('plans_type_check');
    expect(ddl).toContain("'postgame_parent_texts'");
  });

  it('uses ALTER TABLE plans + drops-and-recreates the same constraint (matching 044)', () => {
    const { sql } = findMigration();
    const ddl = ddlOnly(sql).toLowerCase();
  ...
  it('preserves every previously allowed plan type (regression guard)', () => {
    const { sql } = findMigration();
    const ddl = ddlOnly(sql).toLowerCase();
    // Spot-check values 034 + 041 + 044 introduced. Every existing plan type
    // must still be on the allow-list — the migration is purely additive.
    for (const t of [
      "'practice'",
      "'gameday'",
      "'parent_report'",
  ...
  it('introduces NO descriptive minor field (COPPA — postgame_parent_texts is an existing-shape write)', () => {
    const { sql } = findMigration();
    const ddl = ddlOnly(sql).toLowerCase();
    // The texts are persisted in the existing plans.content_structured column.
