/**
 * Ticket 0046 — AI contract proof for the sidelineTalkingPoints prompt.
 *
 * The `sidelineTalkingPoints` prompt must produce structurally-valid JSON
 * across at least Anthropic AND one fallback provider (the multi-provider mock
 * strategy from tests/ai/provider-failover.test.ts). The test:
 *   - drives the REAL callAIWithJSON() with mocked Anthropic + OpenAI SDKs
 *   - feeds each a valid two-key sheet with four-key per-entry shapes
 *   - asserts the parsed output validates against sidelineTalkingPointsSchema
 *     for BOTH providers
 *   - iterates the entries array and validates each four-key shape
...
const VALID_SHEET: SidelineTalkingPoints = {
  team_id: 'team-1',
  entries: [
    {
      player_id: 'p-maya',
    ...
    },
    {
      ...
      lead_line: 'Reads the floor before she dribbles — rare at this age.',
...
describe('sidelineTalkingPoints prompt — provider-agnostic JSON contract (ticket 0046)', () => {
  ...
  it('parses against sidelineTalkingPointsSchema when served by Anthropic (primary)', async () => {
    ...
    const result = await callAIWithJSON<SidelineTalkingPoints>(baseOptions, supabase);

    expect(anthropicCreate).toHaveBeenCalledTimes(1);
    const validated = sidelineTalkingPointsSchema.parse(result.parsed);
    expect(validated.entries.length).toBeGreaterThanOrEqual(1);
    // Iterate each entry and validate the four-key shape.
    for (const entry of validated.entries) {
      expect(typeof entry.player_id).toBe('string');
      expect(typeof entry.player_first_name).toBe('string');
      expect(typeof entry.lead_line).toBe('string');
      expect(typeof entry.working_on_line).toBe('string');
      expect(entry.lead_line.length).toBeGreaterThan(1);
      expect(entry.working_on_line.length).toBeGreaterThan(1);
    }
  });

  it('parses against sidelineTalkingPointsSchema when served by the OpenAI fallback', async () => {
  ...
  it('rejects any extra TOP-LEVEL key in the served response (strict allow-list)', () => {
    expect(() =>
      sidelineTalkingPointsSchema.parse({ ...VALID_SHEET, generated_at: 'now' }),
    ).toThrow();
    expect(() =>
      sidelineTalkingPointsSchema.parse({ ...VALID_SHEET, player_full_name: 'Maya Walker' }),
    ).toThrow();
  });
