...
describe('buildWeeklyProgress', () => {
  it('returns exactly weekCount points', () => {
    const points = buildWeeklyProgress([], now, 8);
    expect(points).toHaveLength(8);
  });

  it('labels most-recent point "Now"', () => {
    const points = buildWeeklyProgress([], now, 8);
    expect(points[points.length - 1].weekLabel).toBe('Now');
  });

  it('labels earlier points with week offset', () => {
    const points = buildWeeklyProgress([], now, 8);
    expect(points[0].weekLabel).toBe('7w');
    expect(points[1].weekLabel).toBe('6w');
    expect(points[6].weekLabel).toBe('1w');
  });

  it('counts only positive observations per week', () => {
    const observations: ShareObservation[] = [
      obs({ sentiment: 'positive', created_at: daysAgo(2) }),
      obs({ sentiment: 'positive', created_at: daysAgo(3) }),
      obs({ sentiment: 'needs-work', created_at: daysAgo(3) }),
    ];
    const points = buildWeeklyProgress(observations, now, 8);
    const latest = points[points.length - 1];
    expect(latest.positiveCount).toBe(2);
    expect(latest.totalCount).toBe(3);
  });

  it('places observations in the correct weekly bucket', () => {
    const observations: ShareObservation[] = [
      obs({ sentiment: 'positive', created_at: daysAgo(10) }), // 2 weeks ago bucket
      obs({ sentiment: 'positive', created_at: daysAgo(11) }),
    ];
    const points = buildWeeklyProgress(observations, now, 8);
    // 10 days ago falls in the "2w" bucket (index 6 of 8)
    const twoWeeksAgo = points[points.length - 3]; // index 5
    const oneWeeksAgo = points[points.length - 2]; // index 6
    const thisWeek = points[points.length - 1];    // index 7
    // 10-11 days ago should land in the bucket covering 14-7 days ago
    expect(oneWeeksAgo.positiveCount + twoWeeksAgo.positiveCount).toBe(2);
    expect(thisWeek.positiveCount).toBe(0);
  });

  it('handles empty observations', () => {
    const points = buildWeeklyProgress([], now, 8);
    expect(points.every((p) => p.positiveCount === 0 && p.totalCount === 0)).toBe(true);
  });

  it('custom weekCount', () => {
    const points = buildWeeklyProgress([], now, 4);
...
describe('hasEnoughDataForWeeklyProgress', () => {
  it('returns false when no weeks have data', () => {
    const points = buildWeeklyProgress([], now, 8);
    expect(hasEnoughDataForWeeklyProgress(points)).toBe(false);
  });

  it('returns false when only 1 week has data', () => {
    const observations: ShareObservation[] = [
      obs({ created_at: daysAgo(2) }),
    ];
    const points = buildWeeklyProgress(observations, now, 8);
    expect(hasEnoughDataForWeeklyProgress(points)).toBe(false);
  });

  it('returns true when 2+ distinct weeks have data', () => {
    const observations: ShareObservation[] = [
      obs({ created_at: daysAgo(2) }),
      obs({ created_at: daysAgo(10) }),
    ];
    const points = buildWeeklyProgress(observations, now, 8);
    expect(hasEnoughDataForWeeklyProgress(points)).toBe(true);
  });
});
...
describe('getWeeklyProgressMax', () => {
  it('returns 1 when all points have 0 positiveCount', () => {
    const points = buildWeeklyProgress([], now, 4);
    expect(getWeeklyProgressMax(points)).toBe(1);
  });

  it('returns the highest positiveCount', () => {
    const observations: ShareObservation[] = [
      obs({ sentiment: 'positive', created_at: daysAgo(2) }),
      obs({ sentiment: 'positive', created_at: daysAgo(2) }),
      obs({ sentiment: 'positive', created_at: daysAgo(2) }),
    ];
    const points = buildWeeklyProgress(observations, now, 4);
    expect(getWeeklyProgressMax(points)).toBe(3);
  });
});
...
describe('isProgressTrending', () => {
  it('returns false when fewer than 4 points', () => {
    const points = buildWeeklyProgress([], now, 3);
    expect(isProgressTrending(points)).toBe(false);
  });
  ...
  it('returns true when recent half has higher avg positive count', () => {
    // Older weeks: 0 each; recent weeks: 2 each → trending up
    const recentObs: ShareObservation[] = [
      obs({ sentiment: 'positive', created_at: daysAgo(2) }),
      obs({ sentiment: 'positive', created_at: daysAgo(2) }),
      obs({ sentiment: 'positive', created_at: daysAgo(9) }),
      obs({ sentiment: 'positive', created_at: daysAgo(9) }),
    ];
    const points = buildWeeklyProgress(recentObs, now, 8);
    expect(isProgressTrending(points)).toBe(true);
  });

  it('returns false when prior half has higher avg positive count', () => {
    // Older weeks: 3 each; recent weeks: 0 each → declining, not trending
    const olderObs: ShareObservation[] = [
      obs({ sentiment: 'positive', created_at: daysAgo(30) }),
      obs({ sentiment: 'positive', created_at: daysAgo(30) }),
      obs({ sentiment: 'positive', created_at: daysAgo(37) }),
    ];
    const points = buildWeeklyProgress(olderObs, now, 8);
    expect(isProgressTrending(points)).toBe(false);
  });
});
