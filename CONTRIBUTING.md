...
### P62 — Game-Day Quick Start
- [x] **Game / Scrimmage / Tournament quick-start from home page + ?type= param on sessions/new** — coaches arriving at a game without pre-scheduling previously had to navigate through 6 steps (Sessions → New Session → select Game → fill form → save → navigate to detail); now three compact emoji buttons ("🏀 Game", "⚡ Scrimmage", "🏆 Tournament") appear in a 3-column row below the "Start Practice" primary button when no practice is active and no today session exists, each linking to `/sessions/new?type=<type>` which lands on the form with the correct session type already selected; `sessions/new` now reads the `?type=` URL param via a lazy `useState` initialiser (validated against `SESSION_TYPES` to prevent invalid inputs) so coaches see the Game tile pre-highlighted and can immediately fill in an optional opponent name and tap "Create Session" — 2 taps total instead of 6; also **tappable stats grid**: the three home-page stats cards (Players / Observations / Sessions) now wrap in `<Link>` elements pointing to `/roster`, `/observations`, and `/sessions` respectively, so coaches can navigate to the most common list pages directly from the stat they care about (instead of using the bottom nav or sidebar); zero new API routes, DB tables, or utility modules; no new tests needed (no utility logic)
